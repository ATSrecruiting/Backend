...
from schema.candidates import Address, GetCandidatePersonalInfo, GetCandidateWorkExperience, ListCandidatesFromSessionIdResponse, RegisterCandidateRequest, RegisterCandidateResponse, CVData, ListCandidatesResponse, VerifyWorkExperienceResponse, WorkExpAttachment, WorkExperience, VerificationDetail, VerificationDetailResponse
...
async def create_candidate_account(account_data: RegisterCandidateRequest, db: AsyncSession = Depends(get_db)):
    try:
        ...
        user = User(username=account_data.username, email=account_data.email, password=password_hash, account_type="candidate")
        db.add(user)
        await db.commit()
...
async def list_candidates_by_id(
    ...
    try:
        ...
        session_q = await db.execute(select(TempChatSession).where(TempChatSession.id == session_id))
...
async def get_candidate_work_experience(candidate_id: int, db: AsyncSession = Depends(get_db)):
    try:
        ...
        result = await db.execute(
            select(Candidate.work_experience).filter(Candidate.id == candidate_id).limit(1)
        )
        raw_work_experience_list = result.scalar_one_or_none()

        if raw_work_experience_list is None:
             candidate_exists_result = await db.execute(select(Candidate.id).filter(Candidate.id == candidate_id).limit(1))
        ...
        if isinstance(raw_work_experience_list, list):
            try:
                validated_work_experiences = [
                    WorkExperience.model_validate(json.loads(exp)) if isinstance(exp, str) else WorkExperience.model_validate(exp)
                    for exp in raw_work_experience_list
                ]
            except Exception as validation_error:
                # logger.error(...)
                raise HTTPException(status_code=500, detail=f"Error validating stored work experience data.")
        else:
             return []
        ...
        if all_attachment_ids:
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail="Invalid attachment ID format found.")
            
            attachment_result = await db.execute(
                select(Attachment.id, Attachment.file_path, Attachment.filename)
                .filter(Attachment.id.in_(typed_attachment_ids))
            )
        ...
        if all_recruiter_ids:
            ...
            recruiter_result = await db.execute(
                select(Recruiter.id, Recruiter.first_name, Recruiter.last_name)
        ...
        for exp in validated_work_experiences:
            ...
            if exp.verifications:
                 for verification in exp.verifications: # Iterate through stored VerificationDetail
                      recruiter_name = recruiter_name_dict.get(verification.recruiter_id, "Unknown Recruiter") # Look up name
...
@router.put("/{candidate_id}/work_experience/{work_id}/verify", response_model=VerifyWorkExperienceResponse)
async def verify_work_experience(
    ...
    try:
        if user.account_type != "recruiter" or user.recruiter is None:
            raise HTTPException(status_code=403, detail="Unauthorized access: User is not a recruiter.")
        ...
        try:
             work_experience_data = [
                ...
                if isinstance(exp, str) else WorkExperience.model_validate(exp) 
                for exp in candidate.work_experience
            ]
        except Exception as validation_error:
             # Handle potential JSON decoding or Pydantic validation errors during loading
             raise HTTPException(status_code=400, detail=f"Error parsing work experience data: {validation_error}")

        if not work_experience_data:
            # This case might be less likely if initialized above, but good to keep
            raise HTTPException(status_code=404, detail="Work experience list is empty for this candidate.")
        ...
        if target_exp is None:
             raise HTTPException(status_code=404, detail=f"Work experience with ID {work_id} not found for this candidate.")
        ...
        if not already_verified:
            ...
            updated_work_experience_json = [exp.model_dump_json() for exp in work_experience_data]

            # Save the updated data
            await db.execute(
                update(Candidate)
                .where(Candidate.id == candidate_id)
                .values(work_experience=updated_work_experience_json) # Save the JSON string list
            )
            await db.commit()
